...
// console.log(FPCData2[region.indexOf(temp)]);
  ...
  {
	  for(var j=0;j<FPCData2.length;j++)
	  {
...
function FirstGraphInfo(saltSugarTotal,countryName){
	this.saltSugarTotal=saltSugarTotal;
...
data.on("close" , () =>{
  ...
  for(var k=0;k<SaltSugar.length;k++){
	  saltSugarTotal=parseFloat(SaltSugar[k].salt)+parseFloat(SaltSugar[k].sugar);
	  //console.log(saltSugarTotal);
	  firstGraphInfo.push(new FirstGraphInfo(saltSugarTotal,SaltSugar[k].country));
  }

  console.log(JSON.stringify(firstGraphInfo));
